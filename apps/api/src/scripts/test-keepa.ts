...
async function testKeepaIntegration() {
    ...
    console.log(` Cents to Euros: ${testCents} cents = EUR ${testEuros}`);
    ...
    console.log(` Discount calculation: EUR 59.99 -> EUR 39.99 = ${discount}%`);
    console.log('');
    ...
    try {
        ...
        console.error('[X] Test 3 failed:', error.message);
    }
    console.log('');
    ...
    try {
        ...
        if (elettronicaId) {
            ...
            const products = await keepa.productFinder({
                ...
                current_AMAZON_gte: 2000,  // EUR 20
                current_AMAZON_lte: 10000, // EUR 100
            ...
            if (products.length > 0) {
                products.forEach(p => {
                    console.log(`   - ${p.asin}: ${p.title.substring(0, 50)}... (EUR ${p.currentPrice})`);
                });
            } else {
        ...
        console.error('[X] Test 4 failed:', error.message);
    }
    console.log('');
